 ...
 * horizontally and centered.<br>
 * Unlike FlowLayout, components will not wrap to form another column if there
 * isn't enough space vertically.<br>
 * VerticalLayout can optionally anchor components to the top or bottom of the
 * display area or center them between the top and bottom.
 * 
 * Revision date 12th July 2001
 * 
 * @author Colin Mummery - <i>e-mail: colin_mummery@yahoo.com
 *         Homepage:www.kagi.com/equitysoft - Based on 'FlexLayout' in Java
 *         class libraries Vol 2 Chan/Lee Addison-Wesley 1998</i>
 * @author w.posdorfer - <i>modifications for sturesy</i>
 */
public class VerticalLayout implements LayoutManager
{
     ...
     * component horizontally and aligning it in the center.
     */
    ...
    public void layoutContainer(Container parent)
    {
        ...
        synchronized (parent.getTreeLock())
        {
            ...
            {
                Dimension compDim = c.getPreferredSize();
            ...
            for (Component comp : parent.getComponents())
            {
                ...
                int x = insets.left;
                int width = compDim.width;
                if (alignment == CENTER)
                {
                    x = (parentDim.width - compDim.width) / 2;
                    x = x < 0 ? 0 : x;
                }
                else if (alignment == RIGHT)
                {
                    x = parentDim.width - compDim.width - insets.right;
                }
                else if (alignment == BOTH)
                {
                    x = (parentDim.width - compDim.width) / 2;
                    x = x < insets.left ? insets.left : x;
                    width = parentDim.width - insets.left - insets.right;
                }
                comp.setBounds(x, y, width, compDim.height);
                y += compDim.height + vgap;
            }
        }
    }
